...
import cn.edu.sxau.dormitorymanage.bean.DataCount;
...
public class JsGridReportBase {
	...
	public static int SHEET_MAX_CNT = 10000; // 每个sheet多少条数据
	 ...
	 * @param tableData
	 *            表格数据
	 * @return void <style name="dataset"> case SYSROWNUM%2==0?#row0:#row1; fontsize:9px; </style> <style name="row0"> import(parent); bgcolor:#FFFFFF; </style> <style name="row1"> import(parent); bgcolor:#CAEAFE; </style>
	 */
	public void exportToExcel(String title, String creator, TableData tableData) throws Exception {
		...
		response.setHeader("Content-Disposition", "attachment;filename=".concat(String.valueOf(encodeFileName(request, sFileName))));
	...
	public void exportToExcel(ZipOutputStream zout, ExcelBean bean) throws Exception {
		...
		for (int i = 1; i <= excel_num; i++) {
			...
			sFileName = URLEncoder.encode(sFileName, "UTF-8");// 处理中文文件名的问题
